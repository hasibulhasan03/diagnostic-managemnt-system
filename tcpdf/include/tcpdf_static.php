	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function fopenLocal($filename, $mode) {
		if (strpos($filename, '://') === false && is_file($filename)) {
			// If the filename is a local file path and exists, use it directly
			return fopen($filename, $mode);
		} elseif (stream_is_local($filename) !== true) {
			// If the filename is not a local file path, return false
			return false;
		}
	
		// If the filename is a local file path but doesn't exist, prepend 'file://' and try again
		return fopen('file://' . $filename, $mode);
	}
